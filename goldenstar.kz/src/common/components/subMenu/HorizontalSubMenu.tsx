...
export function HorizontalSubMenu(props: HorizontalSubMenuProps) {
  ...
  return (
    <li
      ...
      {label}
      <ul
        className={`border-softBlack iMac:-top-[2px] iMac:left-[25rem] iMac:w-[25rem] absolute  -top-[2px] left-72 w-[18rem] ${
          isOpened ? '' : 'hidden'
        ...
        {links.map((link, i) => {
          ...
          return (
            <li
              key={link.text}
              className={`hover:bg-gold hover:border-gold border-t-10 hover:text-white ${itemClass}`}>
